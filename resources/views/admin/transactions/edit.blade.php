    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.transactions.update", [$transaction->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('room') ? 'is-invalid' : '' }}" name="room_id" id="room_id" required>
                    @foreach($rooms as $id => $room)
                        <option value="{{ $id }}" {{ (old('room_id') ? old('room_id') : $transaction->room->id ?? '') == $id ? 'selected' : '' }}>{{ $room }}</option>
                    @endforeach
                </select>
                @if($errors->has('room'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('check_in') ? 'is-invalid' : '' }}" type="text" name="check_in" id="check_in" value="{{ old('check_in', $transaction->check_in) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('check_out') ? 'is-invalid' : '' }}" type="text" name="check_out" id="check_out" value="{{ old('check_out', $transaction->check_out) }}" required>
