    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.banks.update", [$bank->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('property') ? 'is-invalid' : '' }}" name="property_id" id="property_id" required>
                    @foreach($properties as $id => $property)
                        <option value="{{ $id }}" {{ (old('property_id') ? old('property_id') : $bank->property->id ?? '') == $id ? 'selected' : '' }}>{{ $property }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('number') ? 'is-invalid' : '' }}" type="number" name="number" id="number" value="{{ old('number', $bank->number) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" name="name" id="name" required>
                    ...
                    @foreach(App\Models\Bank::NAME_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('name', $bank->name) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('name'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cardholder') ? 'is-invalid' : '' }}" type="text" name="cardholder" id="cardholder" value="{{ old('cardholder', $bank->cardholder) }}" required>
